defmodule Skywire.Subscriptions do
  ...
  def create_subscription(attrs \\ %{}) do
    ...
    if changeset.valid? do
      ...
      case OpenSearch.index_subscription(sub.external_id, query_doc) do
        {:ok, %{status: status}} when status in [200, 201] -> 
          IO.puts("✅ Indexed subscription #{sub.external_id}")
        
        {:ok, response} ->
          IO.puts("❌ Failed to index subscription (Status #{response.status}): #{inspect(response.body)}")
          
        error -> 
          IO.puts("❌ Failed to index subscription: #{inspect(error)}")
      end
