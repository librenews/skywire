defmodule Skywire.Firehose.Processor do
  ...
  def init(_opts) do
    ...
    sample_rate = Application.get_env(:skywire, :firehose_sample_rate, 1.0)
    batch_size = Application.get_env(:skywire, :batch_size, 256)
    {:ok, %State{sample_rate: sample_rate, batch_size: batch_size}}
  end

  @impl true
  def handle_cast({:process_event, event}, state) do
    # Sampling Logic: Probabilistic drop
    if state.sample_rate < 1.0 and :rand.uniform() > state.sample_rate do
      {:noreply, state}
    else
      ...
      if new_size >= state.batch_size do
        flush_buffer(new_state)
        {:noreply, %State{sample_rate: state.sample_rate, batch_size: state.batch_size}} # Preserve state
      else
        {:noreply, new_state}
      end
    end
  ...
  def handle_info(:flush, state) do
    if state.buffer_size > 0 do
      flush_buffer(state)
      {:noreply, %State{sample_rate: state.sample_rate, batch_size: state.batch_size}}
    else
  ...
  defp flush_buffer(state) do
    ...
    case Skywire.Search.OpenSearch.bulk_index(events_with_embeddings) do
      {:ok, _resp} ->
         ...
         Logger.info("👉 Calling Matcher.check_matches...")
